    ...
    response <-
      httr::GET(
        paste0(
          url,
          i,
          "&includes=companies%20tags%20comments%20emails%20tels%20positions%20tasks%20tasks_pending&methods=all"
        ),
...
#' @param filter_vector vector to filter by "company_id" or "person_id" (decided by filter_by), not needed if all protocolls should get downloaded
