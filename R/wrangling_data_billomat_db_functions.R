...
clients_columns <- c("id",
                     ...
                     "Ansprechpartner in Rechnung nennen",
                     "Link Rechnungsportal",
                     "Rechnung auf Englisch",
                     "Zusätzliche Rechnungswünsche",
                     "Kunde_kurz_Datev",
                     "abw. RE Debitorennummer",
                     "Debitorennummer (neu)",
                     "vat_number",
                     "due_days",
                     "facturx_identifier",
...
get_db_2_wide_df <- function (db_table_name,
  ...
  df <-
    ...
    # group_by(ids, page, name) %>%
    # summarise(n = n()) %>%
    tidyr::pivot_wider(
      id_cols = c("ids", "page"),
      names_from = "name",
      values_fn = first, # sometimes there are more than one tax value per id
      values_from = "value"
    )
